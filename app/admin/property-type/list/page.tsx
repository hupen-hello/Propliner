...
export default function PropertyTypeList() {
  ...
  const [propertyTypes, setPropertyTypes] = useState([
    ...
    { id: 4, title: "Industrial Property", status: "Active" },
  ]);

  return (
    <div className="max-w-6xl mx-auto pb-10">
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Property Type List</h1>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="text-sm text-gray-700">
              {propertyTypes.map((item, index) => (
                <tr 
                  key={item.id} 
                  className={`border-b border-gray-100 hover:bg-gray-50 transition-colors ${index === propertyTypes.length - 1 ? 'border-none' : ''}`}
                >
